...
class minStockWindow(internalWindows,Ui_MineralStock):
    ...
    def updDatabase(self, ID):
        ...
        self.KMMdatabase.updateData('''
            ...
            SET quantity = ?, cost = ? 
            WHERE fieldID = ? ;''', (undot_me(quant),unmoney_me(cost), ID))
    ...
    def decreaseMinsBPs(self, Item, typeID):
        ...
        if result == 3: #Yes
            ...
            try: #Minerals
                ...
                mineralNames = ("Tritanium","Pyerite", "Mexallon", "Isogen", "Nocxium", "Zydrine", "Megacyte", "Morphite")
                ...
                for mineral, itemneeds, stock in zip(minerals, itemData[10:19], mineralData):
                    if mineral == stock[0]:
                        if itemneeds <= stock[1]:
                            newStock = stock[1]-(itemneeds)
                            self.KMMdatabase.updateData('''
                            ...
                            raise Exception(mineralNames[minerals.index(mineral)])
            ...
            try: #Blueprints
                ...
                if bp[0] == 'BPO':
                    pass
                elif bp[0] == 'BPC' and bp[1] >= itemData[2]:
                    newRuns = bp[1]-itemData[2]
                    self.KMMdatabase.updateData('''
                        UPDATE BPAssets 
                        SET runs = ?
                        WHERE ID = ?;
...
class salesWindow(internalWindows, Ui_Sales):
    ...
    def setupTable(self, filtertype="All"):
        ...
        for saleItem in Items:
            ...
            saleItems.append((str(itemData[0]),
                              str(itemData[1]),
                              str(saleData[0]),
                              QtGui.QTableWidgetItem(str(loc)),
                              str(itemData[2]),
                              str(saleData[2]),
                              QtGui.QTableWidgetItem(str(itemData[3])),
                              money_me(unmoney_me(itemData[4])/itemData[2]),
                              money_me(float(saleData[3])),
                              money_me(unmoney_me(saleData[3])-unmoney_me(itemData[4])/itemData[2]),
                              money_me(((unmoney_me(saleData[3])-unmoney_me(itemData[4])/itemData[2])/(unmoney_me(itemData[4])/itemData[2]))*100)+"%"
                              ))
