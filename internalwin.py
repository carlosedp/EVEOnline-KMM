...
class addCharWindow(internalWindows, Ui_addCharacter):
    """
    Small windows to add a new character
    No use until now because no multi-char support
    """
    def __init__(self, parent):
        internalWindows.__init__(self, parent)
...
class skillsWindow(internalWindows,Ui_CharacterSkills):
    ...
    Can import the skills from EVE-Online XML file via xmlparse.py
    """
    def __init__(self, parent):
        internalWindows.__init__(self, parent)
...
#TODO: Fix this, show capital ship cost
            itemData    = Item.getTableData()
            if capitalship == 0:
...
class salesWindow(internalWindows, Ui_Sales):
        ...
        for row in range(self.salesTable.rowCount()):
            ...
            try:
                additionalData = self.KMMdatabase.queryData('''
                    ...
                    WHERE loc.location = "''' + str(sale_loc) + '\";')
            ...
            self.KMMdatabase.updateData('''UPDATE itemmanufacture 
                SET sale_date = ?, sale_locationID = ? ,quantitysold = ? ,statusID = ?, sell_price = ? 
                WHERE ID = ?;''', (sale_date, locID, qt_sold, statusID, sell_price, ID))
            self.KMMdatabase.commitData()
        self.setupTable()
    ...
    def setupTable(self, filtertype="All"):
        ...
        tableColumns = ["Lot ID.", "Item", "Sale Date","Sale Location", "Qty Built","Qty Sold", 
                        "Status", "Build Cost", "Sale Price", "Margin",  "Margin %"]
        ...
        self.salesTable.setItemDelegate(Delegate(self))

        self.salesTable.verticalHeader().hide()

        saleItems = self.KMMdatabase.queryData('''
            SELECT ID, typeID, quantity, date, PE, BPME, BPPE, locationID, statusID
            FROM itemmanufacture 
        ...
        for saleItem in saleItems:
            ...
            saleData = self.KMMdatabase.queryData('''
                ...
                WHERE item.ID = ''' + str(itemData[0]) + ';')
            saleData = saleData[0]

            if saleData[1]:
                loc = self.KMMdatabase.queryData('''
                    SELECT location FROM locations
                    WHERE ID = ''' + str(saleData[1]) + ';')
                loc = loc[0][0]
            else:
            	loc = ""

            item0 = str(itemData[0])
            item1 = str(itemData[1])
            item2 = str(saleData[0])
            item3 = QtGui.QTableWidgetItem(str(loc))
            item4 = str(itemData[2])
            item5 = str(saleData[2])
            item6 = QtGui.QTableWidgetItem(str(itemData[3]))
            item7 = itemData[4]
            item8 = money_me(float(saleData[3]))
            item9 = money_me(unmoney_me(saleData[3])-unmoney_me(item7))
            item10 = money_me(unmoney_me(item9)/unmoney_me(item7)*100) + "%"

            totalMargin += unmoney_me(saleData[3])-unmoney_me(item7)

            self.salesTable.setItem(row, 0, QtGui.QTableWidgetItem(item0))
            self.salesTable.setItem(row, 1, QtGui.QTableWidgetItem(item1))
            self.salesTable.setItem(row, 2, QtGui.QTableWidgetItem(item2))
            self.salesTable.setItem(row, 3, item3)
            self.salesTable.setItem(row, 4, QtGui.QTableWidgetItem(item4))
            self.salesTable.setItem(row, 5, QtGui.QTableWidgetItem(item5))
            self.salesTable.setItem(row, 6, item6)
            self.salesTable.setItem(row, 7, QtGui.QTableWidgetItem(item7))
            self.salesTable.setItem(row, 8, QtGui.QTableWidgetItem(item8))
            self.salesTable.setItem(row, 9, QtGui.QTableWidgetItem(item9))
            self.salesTable.setItem(row, 10, QtGui.QTableWidgetItem(item10))

            self.salesTable.openPersistentEditor(item3)
            self.salesTable.openPersistentEditor(item6)
